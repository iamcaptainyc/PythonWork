...
def get_hiformer_b_configs():
    ...
    if not os.path.isfile('./weights/swin_tiny_patch4_window7_224.pth'):
        ...
        wget.download("https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth", "/kaggle/working/swin_tiny_patch4_window7_224.pth")    
    cfg.swin_pretrained_path = '/kaggle/working/swin_tiny_patch4_window7_224.pth'
...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size = 224, in_channels=3):
        ...
        self.resnet_layers = nn.ModuleList(resnet.children())[:7] #0-3 初始conv,outc=64, 4---layer1,outc=256, 5---layer2,outc=512, 6---layer3,outc=1024
    ...
    def forward(self, x):
        ...
        for i in range(5):
            x = self.resnet_layers[i](x) #执行到resnet_layers[4] = resnet50.layer1, x/4
